...
control Eg(inout Headers hdrs,
    ...
    action R1(inout Key_R1 evictedKey, inout Value_R1 evictedValue) {
        ...
        bit<32> _val_last_time = ok ? existingValue_f0  : 0;
        bit<32> _val_size = ok ? existingValue_f1  : 0;
        ...
        _tmp_R1_valid = (_pred_3) || (_tmp_R1_valid);
        meta.query_meta.size = _pred_3 ? (_val_size) : (meta.query_meta.size);
        meta.query_meta.last_time = _pred_3 ? (_val_last_time) : (meta.query_meta.last_time);
        _size_a = _pred_3 ? ((_size_a)*(32w0)) : (_size_a);
        _size_b = _pred_3 ? ((32w1)+((32w0)*(_size_b))) : (_size_b);
        _pred_4 = (_pred_1) && (! (_pred_2));
        _size_a = _pred_4 ? ((_size_a)*(32w1)) : (_size_a);
        _size_b = _pred_4 ? ((32w1)+((32w1)*(_size_b))) : (_size_b);
        _val_last_time = (bit<32>)meta.queueing_metadata.enq_timestamp;
        _val_size = ((_size_a)*(_val_size))+(_size_b);
        // Postamble
        ...
        regV_R1_f0.write(hash_table_index, _val_last_time);
        regV_R1_f1.write(hash_table_index, _val_size);
    ...
    // Function body
    meta.query_meta.size_index = (meta.query_meta.size)>>(8w7);
    _tmp_R2_valid = _tmp_R1_valid;
    // Postamble
    ...
    // Template hole: register for each field in value
    register<bit<32>>(32w1024) regV_result_f0;
    ...
    action result(inout Key_result evictedKey, inout Value_result evictedValue) {
        ...
        // Template hole: Populate inKey's fields from the current packet.
        Key_result inKey;
        inKey.f0 = meta.query_meta.size_index;
        ...
        // Template hole: Get value if there is no eviction, pass this on to update code
        bit<32> _val_sum = ok ? existingValue_f0  : 0;
        ...
        _sum_a = (_sum_a)*(32w1);
        _sum_b = (32w1)+((32w1)*(_sum_b));
        _val_sum = ((_sum_a)*(_val_sum))+(_sum_b);
        // Postamble
        meta.query_meta._result_valid = _tmp_result_valid ? (1w1) : (1w0);


        // Template hole: Write the inKey and the updated (or initial) value into the register.
        regK_result_f0.write(hash_table_index, inKey.f0);

        regV_result_f0.write(hash_table_index, _val_sum);
