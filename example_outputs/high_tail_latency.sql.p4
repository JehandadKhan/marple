...
control Eg(inout Headers hdrs,
    ...
    action R1(inout Key_R1 evictedKey, inout Value_R1 evictedValue) {
        ...
        Key_R1 inKey;
        inKey.f0 = (bit<32>)standard_meta.egress_port;
        ...
        bit<32> _val_tot = ok ? existingValue_f0  : 0;
        bit<32> _val_high = ok ? existingValue_f1  : 0;
        ...
        _tot_b = 32w0;
        _pred_1 = true;
        _pred_2 = ((bit<32>)meta.queueing_metadata.enq_qdepth) > (32w1000);
        _pred_3 = (_pred_1) && (_pred_2);
        _high_a = _pred_3 ? ((_high_a)*(32w1)) : (_high_a);
        _high_b = _pred_3 ? ((32w1)+((32w1)*(_high_b))) : (_high_b);
        _tot_a = (_tot_a)*(32w1);
        _tot_b = (32w1)+((32w1)*(_tot_b));
        _tmp_R1_valid = (_pred_1) || (_tmp_R1_valid);
        meta.query_meta.high = _pred_1 ? (_val_high) : (meta.query_meta.high);
        meta.query_meta.tot = _pred_1 ? (_val_tot) : (meta.query_meta.tot);
        _val_tot = ((_tot_a)*(_val_tot))+(_tot_b);
        _val_high = ((_high_a)*(_val_high))+(_high_b);
        // Postamble
        ...
        regV_R1_f0.write(hash_table_index, _val_tot);
        regV_R1_f1.write(hash_table_index, _val_high);
    ...
    // Function body
    _tmp_R2_valid = (_tmp_R1_valid) && (((meta.query_meta.high)*(32w100)) > (meta.query_meta.tot));
    // Postamble
