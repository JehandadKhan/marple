...
control Eg(inout Headers hdrs,
    ...
    // 2. inKey and evictedKey are of type struct Key_ewma_query. evictedValue is of type struct Value_ewma_query.
    ...
    register<bit<32>>(32w1024) regK_ewma_query_f0;
    register<bit<32>>(32w1024) regK_ewma_query_f1;
    register<bit<32>>(32w1024) regK_ewma_query_f2;
    register<bit<32>>(32w1024) regK_ewma_query_f3;
    register<bit<32>>(32w1024) regK_ewma_query_f4;
    register<bit<32>>(32w1024) regK_ewma_query_f5;


    // Template hole: register for each field in value
    register<bit<32>>(32w1024) regV_ewma_query_f0;


    // Template hole: action signature for groupby
    action ewma_query(inout Key_ewma_query evictedKey, inout Value_ewma_query evictedValue) {

        // Template hole: Populate inKey's fields from the current packet.
        Key_ewma_query inKey;
        ...
        regK_ewma_query_f0.read(existingKey_f0, hash_table_index);
        regK_ewma_query_f1.read(existingKey_f1, hash_table_index);
        regK_ewma_query_f2.read(existingKey_f2, hash_table_index);
        regK_ewma_query_f3.read(existingKey_f3, hash_table_index);
        regK_ewma_query_f4.read(existingKey_f4, hash_table_index);
        regK_ewma_query_f5.read(existingKey_f5, hash_table_index);

        regV_ewma_query_f0.read(existingValue_f0, hash_table_index);
        ...
        bit<32> _val_avg = ok ? existingValue_f0  : 0;
        ...
        // Preamble
        bool _tmp_ewma_query_valid;
        // Function body
        bool _pred_1;
        bit<32> alpha;
        bit<32> _avg_a;
        bit<32> _avg_b;
        _tmp_ewma_query_valid = false;
        _avg_a = 32w1;
        _avg_b = 32w0;
        _pred_1 = true;
        alpha = (32w3)>>(8w4);
        _avg_a = (_avg_a)*(alpha);
        _avg_b = (((32w1)-(alpha))*((meta.common_meta.egress_timestamp)-((bit<32>)meta.queueing_metadata.enq_timestamp)))+((alpha)*(_avg_b));
        _val_avg = ((_avg_a)*(_val_avg))+(_avg_b);
        // Postamble
        meta.query_meta._ewma_query_valid = _tmp_ewma_query_valid ? (1w1) : (1w0);


        // Template hole: Write the inKey and the updated (or initial) value into the register.
        regK_ewma_query_f0.write(hash_table_index, inKey.f0);
        regK_ewma_query_f1.write(hash_table_index, inKey.f1);
        regK_ewma_query_f2.write(hash_table_index, inKey.f2);
        regK_ewma_query_f3.write(hash_table_index, inKey.f3);
        regK_ewma_query_f4.write(hash_table_index, inKey.f4);
        regK_ewma_query_f5.write(hash_table_index, inKey.f5);

        regV_ewma_query_f0.write(hash_table_index, _val_avg);
    ...
    apply {
        ...
        Key_ewma_query  evictedKey_ewma_query;
        Value_ewma_query evictedValue_ewma_query;
        ewma_query(evictedKey_ewma_query,evictedValue_ewma_query);
