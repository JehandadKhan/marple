...
control Eg(inout Headers hdrs,
    ...
    // 2. inKey and evictedKey are of type struct Key_toq. evictedValue is of type struct Value_toq.
    ...
    register<bit<32>>(32w1024) regK_toq_f0;
    register<bit<32>>(32w1024) regK_toq_f1;
    register<bit<32>>(32w1024) regK_toq_f2;
    register<bit<32>>(32w1024) regK_toq_f3;
    register<bit<32>>(32w1024) regK_toq_f4;
    register<bit<32>>(32w1024) regK_toq_f5;


    // Template hole: register for each field in value
    register<bit<32>>(32w1024) regV_toq_f0;
    register<bit<32>>(32w1024) regV_toq_f1;


    // Template hole: action signature for groupby
    action toq(inout Key_toq evictedKey, inout Value_toq evictedValue) {
        ...
        regK_toq_f0.read(existingKey_f0, hash_table_index);
        regK_toq_f1.read(existingKey_f1, hash_table_index);
        regK_toq_f2.read(existingKey_f2, hash_table_index);
        regK_toq_f3.read(existingKey_f3, hash_table_index);
        regK_toq_f4.read(existingKey_f4, hash_table_index);
        regK_toq_f5.read(existingKey_f5, hash_table_index);

        regV_toq_f0.read(existingValue_f0, hash_table_index);
        regV_toq_f1.read(existingValue_f1, hash_table_index);
        ...
        bit<32> _val_last_time = ok ? existingValue_f1  : 0;
        ...
        bit<32> _count_a;
        bit<32> _count_b;
        _tmp_toq_valid = false;
        _count_a = 32w1;
        _count_b = 32w0;
        _pred_1 = true;
        timediff = ((bit<32>)meta.queueing_metadata.enq_timestamp)-(_val_last_time);
        _pred_2 = ((timediff) > (32w200)) && ((timediff) < (32w320));
        ...
        // Postamble
        meta.query_meta._toq_valid = _tmp_toq_valid ? (1w1) : (1w0);


        // Template hole: Write the inKey and the updated (or initial) value into the register.
        regK_toq_f0.write(hash_table_index, inKey.f0);
        regK_toq_f1.write(hash_table_index, inKey.f1);
        regK_toq_f2.write(hash_table_index, inKey.f2);
        regK_toq_f3.write(hash_table_index, inKey.f3);
        regK_toq_f4.write(hash_table_index, inKey.f4);
        regK_toq_f5.write(hash_table_index, inKey.f5);

        regV_toq_f0.write(hash_table_index, _val_count);
        regV_toq_f1.write(hash_table_index, _val_last_time);
    ...
    apply {
        ...
        Key_toq  evictedKey_toq;
        Value_toq evictedValue_toq;
        toq(evictedKey_toq,evictedValue_toq);
