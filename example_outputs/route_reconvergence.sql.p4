...
control Eg(inout Headers hdrs,
    ...
    // 2. inKey and evictedKey are of type struct Key_temp. evictedValue is of type struct Value_temp.
    ...
    register<bit<32>>(32w1024) regK_temp_f0;
    register<bit<32>>(32w1024) regK_temp_f1;
    register<bit<32>>(32w1024) regK_temp_f2;
    register<bit<32>>(32w1024) regK_temp_f3;
    register<bit<32>>(32w1024) regK_temp_f4;
    register<bit<32>>(32w1024) regK_temp_f5;


    // Template hole: register for each field in value
    register<bit<32>>(32w1024) regV_temp_f0;
    register<bit<32>>(32w1024) regV_temp_f1;


    // Template hole: action signature for groupby
    action temp(inout Key_temp evictedKey, inout Value_temp evictedValue) {
        ...
        regK_temp_f0.read(existingKey_f0, hash_table_index);
        regK_temp_f1.read(existingKey_f1, hash_table_index);
        regK_temp_f2.read(existingKey_f2, hash_table_index);
        regK_temp_f3.read(existingKey_f3, hash_table_index);
        regK_temp_f4.read(existingKey_f4, hash_table_index);
        regK_temp_f5.read(existingKey_f5, hash_table_index);

        regV_temp_f0.read(existingValue_f0, hash_table_index);
        regV_temp_f1.read(existingValue_f1, hash_table_index);
        ...
        bit<32> _val_last_path = ok ? existingValue_f0  : 0;
        bit<32> _val_ch_count = ok ? existingValue_f1  : 0;
        ...
        bit<32> _ch_count_a;
        bit<32> _ch_count_b;
        _tmp_temp_valid = false;
        _ch_count_a = 32w1;
        _ch_count_b = 32w0;
        _pred_1 = true;
        _pred_2 = (meta.common_meta.pktpath) != (_val_last_path);
        _pred_3 = (_pred_1) && (_pred_2);
        _ch_count_a = _pred_3 ? ((_ch_count_a)*(32w1)) : (_ch_count_a);
        _ch_count_b = _pred_3 ? ((32w1)+((32w1)*(_ch_count_b))) : (_ch_count_b);
        _val_last_path = meta.common_meta.pktpath;
        _val_ch_count = ((_ch_count_a)*(_val_ch_count))+(_ch_count_b);
        // Postamble
        meta.query_meta._temp_valid = _tmp_temp_valid ? (1w1) : (1w0);


        // Template hole: Write the inKey and the updated (or initial) value into the register.
        regK_temp_f0.write(hash_table_index, inKey.f0);
        regK_temp_f1.write(hash_table_index, inKey.f1);
        regK_temp_f2.write(hash_table_index, inKey.f2);
        regK_temp_f3.write(hash_table_index, inKey.f3);
        regK_temp_f4.write(hash_table_index, inKey.f4);
        regK_temp_f5.write(hash_table_index, inKey.f5);

        regV_temp_f0.write(hash_table_index, _val_last_path);
        regV_temp_f1.write(hash_table_index, _val_ch_count);
    ...
    apply {
        ...
        Key_temp  evictedKey_temp;
        Value_temp evictedValue_temp;
        temp(evictedKey_temp,evictedValue_temp);
