...
control Eg(inout Headers hdrs,
    ...
    action tcps() {
    // Preamble
    bool _tmp_tcps_valid;
    // Function body
    _tmp_tcps_valid = (((bit<32>)hdrs.ip.protocol) == (32w6)) && (((meta.common_meta.switchId) == (32w1)) || ((meta.common_meta.switchId) == (32w2)));
    // Postamble
    meta.query_meta._tcps_valid = _tmp_tcps_valid ? (1w1) : (1w0);

    } 
    action tslots() {
    // Preamble
    bool _tmp_tslots_valid;
    // Function body
    meta.query_meta.epoch = ((bit<32>)meta.queueing_metadata.enq_timestamp)>>(8w7);
    _tmp_tslots_valid = true;
    // Postamble
    meta.query_meta._tslots_valid = _tmp_tslots_valid ? (1w1) : (1w0);

    } 
    action joined() {
    // Preamble
    bool _tmp_joined_valid;
    bool _tmp_tcps_valid;
    bool _tmp_tslots_valid;
    _tmp_tcps_valid = (meta.query_meta._tcps_valid) == (1w1);
    _tmp_tslots_valid = (meta.query_meta._tslots_valid) == (1w1);
    // Function body
    _tmp_joined_valid = (_tmp_tcps_valid) && (_tmp_tslots_valid);
    // Postamble
    meta.query_meta._joined_valid = _tmp_joined_valid ? (1w1) : (1w0);
    ...
    // 2. inKey and evictedKey are of type struct Key_oos. evictedValue is of type struct Value_oos.
    ...
    register<bit<32>>(32w1024) regK_oos_f0;
    register<bit<32>>(32w1024) regK_oos_f1;
    register<bit<32>>(32w1024) regK_oos_f2;
    register<bit<32>>(32w1024) regK_oos_f3;
    register<bit<32>>(32w1024) regK_oos_f4;
    register<bit<32>>(32w1024) regK_oos_f5;


    // Template hole: register for each field in value
    register<bit<32>>(32w1024) regV_oos_f0;
    register<bit<32>>(32w1024) regV_oos_f1;


    // Template hole: action signature for groupby
    action oos(inout Key_oos evictedKey, inout Value_oos evictedValue) {
        ...
        inKey.f4 = meta.common_meta.switchId;
        ...
        regK_oos_f0.read(existingKey_f0, hash_table_index);
        regK_oos_f1.read(existingKey_f1, hash_table_index);
        regK_oos_f2.read(existingKey_f2, hash_table_index);
        regK_oos_f3.read(existingKey_f3, hash_table_index);
        regK_oos_f4.read(existingKey_f4, hash_table_index);
        regK_oos_f5.read(existingKey_f5, hash_table_index);

        regV_oos_f0.read(existingValue_f0, hash_table_index);
        regV_oos_f1.read(existingValue_f1, hash_table_index);
        ...
        bit<32> _val_count = ok ? existingValue_f0  : 0;
        bit<32> _val_lastseq = ok ? existingValue_f1  : 0;
        ...
        bool _tmp_oos_valid;
        bool _tmp_joined_valid;
        _tmp_joined_valid = (meta.query_meta._joined_valid) == (1w1);
        // Function body
        bool _pred_1;
        bool _pred_2;
        bool _pred_3;
        bit<32> _count_a;
        bit<32> _count_b;
        _tmp_oos_valid = false;
        _count_a = 32w1;
        _count_b = 32w0;
        _pred_1 = _tmp_joined_valid;
        _pred_2 = (_val_lastseq) != (hdrs.tcp.seqNo);
        _pred_3 = (_pred_1) && (_pred_2);
        _count_a = _pred_3 ? ((_count_a)*(32w1)) : (_count_a);
        _count_b = _pred_3 ? ((32w1)+((32w1)*(_count_b))) : (_count_b);
        _tmp_oos_valid = (_pred_3) || (_tmp_oos_valid);
        meta.query_meta.count = _pred_3 ? (_val_count) : (meta.query_meta.count);
        meta.query_meta.lastseq = _pred_3 ? (_val_lastseq) : (meta.query_meta.lastseq);
        _val_lastseq = (hdrs.tcp.seqNo)+(meta.common_meta.payload_length);
        _val_count = ((_count_a)*(_val_count))+(_count_b);
        // Postamble
        meta.query_meta._oos_valid = _tmp_oos_valid ? (1w1) : (1w0);


        // Template hole: Write the inKey and the updated (or initial) value into the register.
        regK_oos_f0.write(hash_table_index, inKey.f0);
        regK_oos_f1.write(hash_table_index, inKey.f1);
        regK_oos_f2.write(hash_table_index, inKey.f2);
        regK_oos_f3.write(hash_table_index, inKey.f3);
        regK_oos_f4.write(hash_table_index, inKey.f4);
        regK_oos_f5.write(hash_table_index, inKey.f5);

        regV_oos_f0.write(hash_table_index, _val_count);
        regV_oos_f1.write(hash_table_index, _val_lastseq);
    ...
    apply {
        ...
        Key_oos  evictedKey_oos;
        Value_oos evictedValue_oos;
        oos(evictedKey_oos,evictedValue_oos);
